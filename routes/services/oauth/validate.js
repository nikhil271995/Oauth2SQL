...
validate.password = (user, password) => new Promise(function (callback, reject) {
    validate.userExists(user);
    db.users.fetchPassword(user.password_id)
        .then((passwordObject) => {
            if (!isValidPassword(passwordObject.password_hash, password)) {
                callback(null);
            }
            else {
                callback(user);
...
validate.refreshToken = (token, refreshToken, client) => {
    ...
    if (client.id.toString() !== token.clientID.toString()) {
...
let isValidPassword = function (passwordHash, password) {
    try {
